...
func LoginHandler(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	if r.Method == http.MethodPost {
		...
		err := db.QueryRow("SELECT id, password FROM users WHERE email = ? OR username = ?", identifier, identifier).Scan(&dbID, &dbPasswordHash)
		...
		_, err = db.Exec("INSERT INTO usersessions (user_id, session_token, expires_at) VALUES (?, ?, ?)", dbID, sessionToken, time.Now().Add(24*time.Hour))
		if err != nil {
